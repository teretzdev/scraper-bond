...
async function main() {
  try {
    ...
    for (let i = 0; i < numPages; i++) {
  	const entry = await pdfParse(pdfPath, { page: i + 1 }); // pass the current page number to the parsePDF function
  	await Bond.create(entry);
  	console.log(`Bond entry for page ${i+1}} has been saved to the database.`);
    }
  } catch (error) {
